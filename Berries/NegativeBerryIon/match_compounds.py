...
# --- CONFIGURATION ---
COMPOUNDS_FILE = "Berry_Comps_Neg_Perfect_Output.xlsx"  # The file we just generated
BERRY_MAPPING_FILE = "Study_Info_Berry_Neg_Filtered.xlsx"       # File containing the mapping of Study File IDs to Berry names
OUTPUT_FILE = "Neg_Matched_Berry_Report.xlsx"

print("Phase 1: Loading data files...")
df_compounds = pd.read_excel(COMPOUNDS_FILE)
df_mapping = pd.read_excel(BERRY_MAPPING_FILE)
...
mapping_veg_col = next((c for c in df_mapping.columns if "berry" in c.lower() or "identifier" in c.lower()), None)
...
# Rename the matched column to exactly "Berry"
merged_df = merged_df.rename(columns={mapping_veg_col: "Berry"})
...
# Fill any unmapped berry entries with "Unknown" just in case
final_df['Berry'] = final_df['Berry'].fillna("Unknown")
